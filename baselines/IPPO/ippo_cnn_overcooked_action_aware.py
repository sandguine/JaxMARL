...
class Transition(NamedTuple):
    ...
    action: jnp.ndarray
    coplayer_action: jnp.ndarray # new field to store the actions of the coplayers
    value: jnp.ndarray
    reward: jnp.ndarray
...
# make RunnerState a NamedTuple for readability
...
# helper functions to augment observations space with co-player actions
def augment_observation(obs_batch, coplay_actions):
    """
    Concatenates the original observation batch with co-player actions.
    Adjusts the axis to match observation space structure.
    """
    return jnp.concatenate([obs_batch, coplay_actions], axis=-1)  # Adjust axis based on observation shape

def get_coplayer(agent_id):
    """
    Returns the ID of the co-player based on the current agent ID.
    Assumes agent IDs are in format 'agent_0', 'agent_1', etc.
    """
    # Extract agent number and compute co-player number
    agent_num = int(agent_id.split('_')[1])
    coplayer_num = (agent_num + 1) % 2  # For 2-agent system
    
    # Return formatted agent ID string
    return f"agent_{coplayer_num}"
...
def make_train(config):
    ...
    def train(rng):
        ...
        # Create init_x with the augmented shape: add an extra channel for action
        obs_shape = list(env.observation_space().shape)
        obs_shape[-1] += 1  # Add one channel for action
        ...
        # Initialize last_actions to store the observed actions of the coplayers
        last_actions = {a: jnp.zeros(env.action_space().shape) for a in env.agents}

        # TRAIN LOOP
        def _update_step(runner_state, unused):
            # COLLECT TRAJECTORIES: update this function to include coplayer actions
