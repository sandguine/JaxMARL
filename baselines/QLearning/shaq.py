...
def make_train(config, env):
    ...
    def train(rng):
        ...
        target_network_params_agent = jax.tree_util.tree_map(lambda x: jnp.copy(x), train_state_agent.params)
        target_network_params_mixer = jax.tree_util.tree_map(lambda x: jnp.copy(x), train_state_mixer.params)
        ...
        def _update_step(runner_state, unused):
            ...
            def _loss_fn(params_agent, params_mixer, target_network_params_agent, target_network_params_mixer, init_hstate, learn_traj):
                ...
                # get the max_filters
                max_filters = jax.tree_util.tree_map(
            ...
            target_network_params_agent = jax.lax.cond(
                ...
                lambda _: jax.tree_util.tree_map(lambda x: jnp.copy(x), train_state_agent.params),
                lambda _: target_network_params_agent,
                operand=None
            )
            target_network_params_mixer = jax.lax.cond(
                time_state['updates'] % config['TARGET_UPDATE_INTERVAL'] == 0,
                lambda _: jax.tree_util.tree_map(lambda x: jnp.copy(x), train_state_mixer.params),
