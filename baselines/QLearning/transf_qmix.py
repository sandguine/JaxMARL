...
def tree_mean(tree):
    return jnp.array(
        jax.tree_util.tree_leaves(jax.tree_util.tree_map(lambda x: x.mean(), tree))
    ).mean()
...
def make_train(config, env):
    ...
    def train(rng):
        ...
        copy_tree = lambda tree: jax.tree_util.tree_map(lambda x: jnp.copy(x), tree)
        ...
        def _update_step(runner_state, unused):
            ...
            def _env_step(step_state, unused):
                ...
                obs, env_state, rewards, dones, infos = wrapped_env.batch_step(key_s, env_state, actions)
                # reward scaling
                rewards = jax.tree_util.tree_map(lambda x:config.get("REW_SCALE", 1)*x, rewards)
